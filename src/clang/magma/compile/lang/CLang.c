#include "CLang.h"
struct Rule createCRootRule(){return new TypeRule(, CommonLang.createBlockRule(createCRootSegmentRule()));
}
struct Rule createCRootSegmentRule(){return new OrRule(Lists.of(
                createIncludeRule(),
...
struct TypeRule createExpansionRule(){StringRule name = new StringRule();
        ...
        return new TypeRule(, new PrefixRule(, new SuffixRule(childRule, )));
}
struct TypeRule createFunctionRule(){OrRule definitionRule = createDefinitionsRule();
        ...
        return new TypeRule(, new InfixRule(definition, , block1, new FirstLocator()));
}
...
struct OrRule createDefinitionsRule(){return new OrRule(Lists.of(
...
struct Rule createIncludeRule(){NodeListRule path = new NodeListRule(, new CharDivider(), new StringRule());
