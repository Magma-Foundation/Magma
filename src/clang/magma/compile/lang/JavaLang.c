...
return new TypeRule(, CommonLang.createBlockRule(createJavaRootSegmentRule()));}
...
return new TypeRule(, new SuffixRule(createDefinitionRule(createTypeRule()), ));}
struct Rule createMethodRule(){
Rule definition = new NodeRule(, createTypedDefinitionRule());
...
return new TypeRule(, createDefinitionRule(createTypeRule()));}
...
return new TypeRule(, new SuffixRule(new NodeRule(, type), ));}
struct Rule createNamedWithTypeParams(){
Rule name = createSymbolRule();
...
NodeListRule namespace = new NodeListRule(, new CharDivider(), new StringRule());
