#include "JavaLang.h"
struct Rule createJavaRootRule(){return new TypeRule(, CommonLang.createBlockRule(createJavaRootSegmentRule()));
}
struct OrRule createJavaRootSegmentRule(){return new OrRule(Lists.of(
...
struct TypeRule createRecordRule(){Rule namedWithTypeParams = createNamedWithTypeParams();
        ...
        return new TypeRule(, new InfixRule(new StringRule(), , right, new FirstLocator()));
}
struct TypeRule createInterfaceRule(){Rule namedWithTypeParams = createNamedWithTypeParams();
        ...
        return new TypeRule(, new InfixRule(new StringRule(), , contentRule, new FirstLocator()));
}
struct TypeRule createClassRule(){Rule namedWithTypeParams = createNamedWithTypeParams();
        ...
        Rule right1 = createContentRule(beforeContent, createClassMemberRule());
        return new TypeRule(, new InfixRule(new StringRule(), , right1, new FirstLocator()));
}
struct Rule createClassMemberRule(){return new OrRule(Lists.of(
                createWhitespaceRule(),
                createMethodRule(),
...
struct TypeRule createInitializationRule(){Rule definitionRule = createDefinitionRule(createTypeRule());
        ...
        return new TypeRule(, new SuffixRule(new InfixRule(new NodeRule(, definition), , new StringRule(), new FirstLocator()), ));
}
struct TypeRule createDefinitionStatementRule(){return new TypeRule(, new SuffixRule(createDefinitionRule(createTypeRule()), ));
}
struct Rule createMethodRule(){Rule definition = new NodeRule(, createTypedDefinitionRule());
        ...
        return new TypeRule(, new InfixRule(definition, , withParams, new FirstLocator()));
}
struct TypeRule createTypedDefinitionRule(){return new TypeRule(, createDefinitionRule(createTypeRule()));
}
