...
struct InfixRule createContentRule(struct Rule beforeContent, struct Rule childRule){Rule children1 = createBlockRule(childRule);
        ...
        return new TypeRule(, new StripRule(rule));}struct TypeRule createWhileRule(){return new TypeRule(, new StripRule(new PrefixRule(, new StringRule())));}struct Rule createGenericRule(struct Rule type){Rule typeArguments = new NodeListRule(, new FoldingDivider(new ValueFolder()), type);
