...
Rule children = new NodeListRule(, new FoldingDivider(new DecoratedFolder(new StatementFolder())), childRule);
        return new NodeRule(, new TypeRule(, children));}
struct TypeRule createSymbolTypeRule(){
return new TypeRule(, createSymbolRule());}
...
NodeListRule modifiers = new NodeListRule(, new CharDivider(), new StringRule());
...
return new NodeListRule(, new FoldingDivider(new ValueFolder()), new OrRule(Lists.of(
                createWhitespaceRule(),
                definition
        )));}
struct TypeRule createWhitespaceRule(){
return new TypeRule(, new StripRule(new EmptyRule()));}
struct TypeRule createAssignmentRule(){
return new TypeRule(, new SuffixRule(new InfixRule(new StringRule(), , new StringRule(), new FirstLocator()), ));}
struct TypeRule createInvocationRule(){
return new TypeRule(, new SuffixRule(new StringRule(), ));}
struct TypeRule createElseRule(){
return new TypeRule(, new StripRule(new PrefixRule(, new StringRule())));}
...
PrefixRule rule = new PrefixRule(, new SuffixRule(new StringRule(), ));
        return new TypeRule(, new StripRule(rule));}
struct TypeRule createWhileRule(){
return new TypeRule(, new StripRule(new PrefixRule(, new StringRule())));}
struct Rule createGenericRule(struct Rule type){
Rule typeArguments = new NodeListRule(, new FoldingDivider(new ValueFolder()), type);
