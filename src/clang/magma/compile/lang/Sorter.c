...
struct Result_Node_CompileError afterPass(struct State state, struct Node node){if (!node.is()) return new Ok<>(node);
...
struct Tuple_List__Node_List__Node foldIntoBuckets(struct Tuple_List__Node_List__Node tuple, struct Node node){if (node.is() || node.is() || node.is())
