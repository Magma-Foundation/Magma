...
struct Node asRoot(struct List__Node left){Node node1 = new MapNode().withNodeList(, left);
        return new MapNode().withNode(, node1);}struct Result_Node_CompileError afterPass(struct State state, struct Node node){if (!node.is()) return new Ok<>(node);
        ...
        return new Ok<>(separated);}struct Tuple_List__Node_List__Node foldIntoBuckets(struct Tuple_List__Node_List__Node tuple, struct Node node){if (node.is() || node.is() || node.is())
        ...
        return new Tuple<>(tuple.left(), tuple.right().add(node));}struct Result_Node_CompileError beforePass(struct State state, struct Node node){return new Ok<>(node);}