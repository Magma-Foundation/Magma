...
struct Tuple_Node_FlattenCache flattenNode(struct FlattenCache cache, struct Node element){if (!element.is()) return new Tuple<>(element, cache);
...
struct Result_Node_CompileError afterPass(struct State state, struct Node node){return new Ok<>(afterPass0(node));
}
struct Result_Node_CompileError beforePass(struct State state, struct Node node){return new Ok<>(node);
}
