...
struct Tuple_Node_FlattenCache flattenNode(struct FlattenCache cache, struct Node element){if (!element.is("group")) return new Tuple<>(element, cache);

        Node child = element.findNode("child").orElse(new MapNode());
        Tuple<Node, FlattenCache> newChild = flattenNode(cache, child);

        FlattenCache groupCategories = element.streamNodeLists().foldWithInitial(newChild.right(), FlattenGroup::flattenCategory);return Tuple_(newChild.left(), groupCategories);
}
struct FlattenCache flattenCategory(struct FlattenCache current, struct Tuple_String_List__Node category){return current.appendCategory(category.left(), category.right());
}
struct FlattenCache flattenNodeList(struct FlattenCache cache, struct Tuple_String_List__Node property){String propertyKey = property.left();
...
struct Tuple_List__Node_FlattenCache flattenNodeListElement(struct Tuple_List__Node_FlattenCache tuple, struct Node node){List_<Node> currentPropertyValues = tuple.left();
        FlattenCache currentCache = tuple.right();

        Tuple<Node, FlattenCache> flattened = flattenNode(currentCache, node);return Tuple_(currentPropertyValues.add(flattened.left()), flattened.right());
}
struct Node afterPass0(struct Node node){FlattenCache cache = new FlattenCache();
