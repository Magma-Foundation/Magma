...
struct String format(int depth, int index, struct List__CompileError sorted){return  + .repeat(depth) + (index + 1) +  + sorted.get(index).format(depth + 1);
}
int depth(){return 1 + errors.stream()
