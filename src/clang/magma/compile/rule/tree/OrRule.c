#include "OrRule.h"
Result<struct T, struct CompileError> apply(Function<struct Rule, Result<struct T, struct CompileError>> applicator, Supplier<struct Context> context}{return rules.stream()
