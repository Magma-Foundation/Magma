#include "NodeRule.h"
auto __lambda0__(){return MapNode();
}
auto __lambda1__(){return CompileError("Failed to attach node '" + propertyKey + "'", StringContext(input), Lists.of(err));
}
auto __lambda2__(){return (CompileError("Node '" + propertyKey + "' not present", NodeContext(node)));
}
