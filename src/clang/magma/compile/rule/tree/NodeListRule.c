...
struct Result_Node_CompileError parse(struct String input){List_<String> segments = divider.divide(input);
        ...
        return segments.stream()
                ...
                .mapErr(err -> new CompileError( + propertyKey + , new NodeContext(node), Lists.of(err)));}struct Result_String_CompileError generateChildren(struct List__Node children){return children.stream().foldToResult(, (current, element) -> childRule
                .generate(element)
