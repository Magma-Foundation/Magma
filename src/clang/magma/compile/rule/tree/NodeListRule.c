...
Result<struct Node, struct CompileError> parse(struct String input}{List_<String> segments = divider.divide(input);
        ...
        return segments.stream()
                ...
                .mapErr(err -> new CompileError( + propertyKey + , new NodeContext(node), Lists.of(err)));}Result<struct String, struct CompileError> generateChildren(List_<struct Node> children}{return children.stream().foldToResult(, (current, element) -> childRule
                .generate(element)
