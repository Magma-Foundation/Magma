...
struct public InfixRule(struct Rule left, struct String infix, struct Rule right, struct Locator locator}{this.left = left;
        this.infix = infix;
        this.right = right;
        this.locator = locator;}Result<struct Node, struct CompileError> parse(struct String input}{return locator.locate(input, infix).map(index -> {
        ...
        });}Result<struct String, struct CompileError> generate(struct Node node}{return left.generate(node).and(() -> right.generate(node)).mapValue(tuple -> tuple.left() + infix + tuple.right());}