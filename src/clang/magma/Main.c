...
magma.result.Result<magma.collect.map.Map_<magma.io.Path_, magma.compile.Node>, magma.ApplicationError> preLoadSources(magma.collect.map.Map_<magma.io.Path_, magma.compile.Node> trees, magma.io.Path_ path){System.out.println("Loading: "+path.asString());
        ...
        State state = new State(namespace, name);if (isPlatformDependent(namespace)) return new Ok<>(trees);return source.readString().mapErr(ApplicationError.new).flatMapValue(__lambda3__.preLoad(input, state).mapErr(ApplicationError.new)).mapValue(__lambda4__.with(path, node));
}
magma.result.Result<magma.collect.list.List_<magma.io.Path_>, magma.ApplicationError> postLoadTree(magma.collect.list.List_<magma.io.Path_> relatives, magma.option.Tuple<magma.io.Path_, magma.compile.Node> pathNodeTuple){Path_ path = pathNodeTuple.left();
        ...
        Source source = new PathSource(SOURCE_DIRECTORY, path);
        List_<String> namespace = source.computeNamespace();return Compiler.postLoad(State(namespace, source.computeName()), tree).mapErr(ApplicationError.new).flatMapValue(__lambda5__(postLoaded, namespace, source.computeName())).mapValue(relatives.addAll);
}
...
magma.result.Result<magma.io.Path_, magma.ApplicationError> writeOutput(magma.io.Path_ parent, String name, String extension, String output){return parent.resolve(name+extension).writeString(output).map(ApplicationError.new).match(Err.new, __lambda8__);
}
...
int isPlatformDependent(magma.collect.list.List_<String> namespace){return namespace.findFirst().filter(__lambda9__.equals("jvm")).isPresent();
}
