import { Import } from "../../magma/app/Import";
...
class CompileState {
	...
	hasImport(child: string): boolean {
		return this.imports.query().filter((node: Import) => Strings.equalsTo(node.child(), child)).next().isPresent();
	}
