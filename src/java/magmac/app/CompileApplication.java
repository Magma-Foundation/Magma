...
public final class CompileApplication implements Application {
    ...
    public Option<Error> run() {
        return this.sources.readAll()
                ...
                .match((UnitSet<String> units) -> this.compileAndWrite(units), (ApplicationError value) -> new Some<>(value));
    }
    ...
    private Option<Error> compileAndWrite(UnitSet<String> units) {
        return this.compiler.compile(units).result()
                ...
                .match((UnitSet<String> outputs) -> this.targets.writeAll(outputs).map((IOException throwable) -> new ThrowableError(throwable)).map((ThrowableError error) -> new ApplicationError(error)), (ApplicationError err) -> new Some<>(err));
    }
}
