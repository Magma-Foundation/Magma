...
R;
{
    ...
    while (true) {
        ...
        current;
        let optional;
        () = this.head.next().map((inner()), folder(finalCurrent, inner));
...
let slice;
() = this.elements.subList(0, this.elements.size() - 1);
let last;
() = this.elements.getLast();
...
let slice;
() = this.elements.subList(1, this.elements.size());
...
let maybeTypeParam;
() = this.typeParams().filter((param()), string);
param.equals(name);
next();
...
{
    let joined;
    () = this.arguments().iterateWithIndices().map((pair()), "arg" + pair.left() + " : " + pair.right().generate()).collect(new Joiner(", ")).orElse("");
...
{
    let joinedArguments;
    () = this.arguments.iterate().map(Type.generate).collect(new Joiner(", ")).orElse("");
...
{
    let joinedArguments;
    () = this.arguments.iterate().map(Type.generate).collect(new Joiner(", ")).map((inner()), "<" + inner + ">").orElse("");
...
{
    let joined;
    () = this.parameters.iterate().map(Definition.generate).collect(new Joiner(", ")).orElse("");
...
{
    let joined;
    () = this.arguments.iterate().map(Value.generate).collect(new Joiner(", ")).orElse("");
...
/* public */ class Main /*  */ {
    main() {
        let parent;
        () = /* Paths */ .get(".", "src", "java", "magma");
        let source;
        () = parent.resolve("Main.java");
        let target;
        () = parent.resolve("main.ts");
        let input;
...
{
    let tuple;
    () = /* compileStatements */ (new CompileState(), input, Main.compileRootSegment);
    let joined;
    () = tuple[0]().structures.iterate().collect(new Joiner()).orElse("");
...
[CompileState, (List)];
{
    return /* getCompileStateListTuple */ (state, input, folder, (state1(), s()));
    new Some(mapper(state1, s));
...
Option < [CompileState, (List)] > {
    let, new: Some(new Tuple2Impl(state, Lists.empty())),
...
Option < [CompileState, string] > {
    return: first(beforeContent, " implements ", (s(), s2()), {}).or(() => {
...
Option < [CompileState, string] > {
    return: first(beforeContent, " extends ", (s(), s2()), {}).or(() => {
...
Option < [CompileState, string] > {
    return: first(beforeContent, "<", (name(), withTypeParams()), {
        return: first(withTypeParams, ">", (typeParamsString(), afterTypeParams()), {
            let
        }(state1(), s()), new Tuple2Impl(state1, s.strip())),
        let, typeParams() { }
    } /* parseValuesOrEmpty */(state, typeParamsString, (state1(), s()), new Some(mapper(state1, s)))),
    return /* assembleStructure */(typeParams) { }
}(), targetInfix, beforeInfix, name, content1, typeParams(), afterTypeParams, params;
;
;
or(() => {
...
let joinedTypeParams;
() = typeParams().collect(new Joiner(", ")).map((inner()), "<" + inner + ">").orElse("");
let statementsTuple;
() = parseStatements(state.pushStructName(name).withTypeParams(typeParams), content, (state0(), input()), /* compileClassSegment */ (state0, input, 1));
/* List<String> parsed1 */ ;
if (params.isEmpty()) {
...
/* else */ {
    let joined;
    () = /* joinValues */ ( /* retainDefinitions */(params));
    let constructorIndent;
    () = /* createIndent */ (1);
    statementsTuple.right().addFirst(constructorIndent + "constructor (" + joined + ") {" + constructorIndent + "}\n");
}
let parsed2;
() = /* parsed1 */ .iterate().collect(new Joiner()).orElse("");
let generated;
() = /* generatePlaceholder */ (beforeInfix.strip()) + targetInfix + name + joinedTypeParams + /* generatePlaceholder */ (afterTypeParams) + " {" + parsed2 + "\n}\n";
...
Option < [CompileState, string] > {
    return: first(input, "(", (definitionString(), withParams()), {
        return: first(withParams, ")", (parametersString(), rawContent()), {
            return /* parseDefinition(state, definitionString).<Tuple2<CompileState, Header>>map */() { }
        }(tuple()), new Tuple2Impl(tuple[0](), tuple[1]())).or(() => /* parseConstructor */ (state, definitionString)).flatMap((definitionTuple()), {
            let, definitionState() { }, definitionTuple, : .left(),
            let, header() { }, definitionTuple, : .right(),
            let, parametersTuple() { }
        } /* parseParameters */(definitionState, parametersString)),
        let, rawParameters() { }, parametersTuple, : .right(),
        let, parameters() { }
    } /* retainDefinitions */(rawParameters)),
    let, joinedParameters() { }
} /* joinValues */(parameters);
let content;
() = rawContent.strip();
let indent;
() = /* createIndent */ (depth);
let paramTypes;
() = parameters.iterate().map(Definition.type).collect(new ListCollector());
let toDefine;
() = header.createDefinition(paramTypes);
let generatedHeader;
() = header.generateWithParams(joinedParameters);
if (content.equals(";")) {
...
if (content.startsWith("{") && content.endsWith("}")) {
    ...
    let statementsTuple;
    () = /* compileFunctionSegments */ (parametersTuple.left().withDefinitions(parameters), substring, depth);
    let generated;
    () = indent + generatedHeader + " {" + statementsTuple.right() + indent + "}";
...
[CompileState, (List)];
{
    return /* parseValuesOrEmpty */ (state, params, (state1(), s()));
    new Some(/* compileParameter */ (state1, s));
    ;
}
...
Option < [CompileState, string] > {
    return: suffix(stripped, ";", (s()), {
        let, tuple() { }
    } /* compileStatementValue */(state, s, depth)),
...
Option < [CompileState, string] > {
    return: suffix(stripped, "}", (withoutEnd()), {
        return /* split */() { }
    }(), /* toFirst */ (withoutEnd), (beforeContent(), content()), {
        return: suffix(beforeContent, "{", (s()), {
            let, compiled() { },
            let, indent() { }
        } /* createIndent */(depth)),
        let, headerTuple() { }
    } /* compileBlockHeader */(state, s, depth)),
...
[CompileState, string];
{
    ...
    if (stripped.startsWith("return ")) {
        let value;
        () = stripped.substring("return ".length());
        let tuple;
        () = /* compileValue */ (state, value, depth);
    ...
    return first(stripped, "=", (beforeEquals(), valueString()), {
        let, sourceTuple() { }
    } /* compileValue */(state, valueString, depth));
    let sourceState;
    () = sourceTuple.left();
    let sourceString;
    () = sourceTuple.right();
    return /* parseDefinition */ (sourceState, beforeEquals).flatMap((definitionTuple()), {
        ...
        () = /* compileValue */ (sourceState, beforeEquals, depth);
        let destinationState;
        () = destinationTuple.left();
        let destinationString;
        () = destinationTuple.right();
...
Option < [CompileState, Value] > {
    return: last(input, "instanceof", (s(), s2()), {
        let, childTuple() { },
        return /* parseDefinition */(childTuple) { }, : .left(), s2
    }).map((definitionTuple()), {
...
if (strippedBeforeArrow.startsWith("(") && strippedBeforeArrow.endsWith(")")) {
    let parameterNames;
    () = divideAll(strippedBeforeArrow.substring(1, strippedBeforeArrow.length() - 1), Main.foldValueChar).iterate().map(strip).filter((value()), T);
    !value.isEmpty();
    map((name()), T);
    Definition.createSimpleDefinition(name, Unknown);
...
{
    let value1;
    () = compileStatements(state2, strippedValueString.substring(1, strippedValueString.length() - 1), (state1(), input1()), compileFunctionSegment(state1, input1, depth + 1));
    let right;
    () = value1.right();
...
/* else */ {
    let value1;
    () = parseValue(state2, strippedValueString, depth);
...
Option < [CompileState, Value] > {
    return: suffix(input.strip(), ")", (withoutEnd()), {
        return /* split */() { }
    }(), /* toLast */ (withoutEnd, "", Main.foldInvocationStart), (callerWithEnd(), argumentsString()), {
...
Some < [CompileState, Value] > {
    let, callerTuple() { }
} /* invocationHeader */(state, depth, callerString);
let oldCallerState;
() = callerTuple.left();
let oldCaller;
() = callerTuple.right();
let newCaller;
() = /* modifyCaller */ (oldCallerState, oldCaller);
let callerType;
() = /* findCallerType */ (newCaller);
let argumentsTuple;
() = /* parseValuesWithIndices */ (oldCallerState, argumentsString, (currentState(), pair()));
{
    let index;
    () = pair.left();
    let element;
    () = pair.right();
    let expectedType;
    () = callerType.arguments.get(index).orElse(Unknown);
    let withExpected;
    () = currentState.withExpectedType(expectedType);
    let valueTuple;
    () = /* parseArgument */ (withExpected, element, depth);
    let valueState;
    () = valueTuple.left();
    let value;
    () = valueTuple.right();
    let actualType;
    () = valueTuple.left().typeRegister.orElse(Unknown);
...
let argumentsState;
() = argumentsTuple.left();
let argumentsWithActualTypes;
() = argumentsTuple.right();
let arguments;
() = argumentsWithActualTypes.iterate().map(Tuple2.left).map(Main.retainValue).flatMap(Iterators.fromOption).collect(new ListCollector());
let invokable;
() = new Invokable(newCaller, arguments, callerType.returns);
...
[CompileState, Argument];
{
    ...
    let tuple;
    () = parseValue(state, element, depth);
...
FunctionType;
{
    let callerType;
    () => ;
    new FunctionType(Lists.empty(), Unknown);
    /* switch (newCaller) */ {
...
[CompileState, Caller];
{
    if (callerString1.startsWith("new ")) {
        let input1;
        () => ;
        callerString1.substring("new ".length());
        let map;
        () = /* parseType */ (state, input1).map((type()), {
            let, right() { },
    ...
    let tuple;
    () = parseValue(state, callerString1, depth);
    return new Tuple2Impl(tuple[0](), tuple[1]());
}
...
let tuple;
() = parseValue(state, parentString, depth);
let parent;
() = tuple[1]();
let parentType;
() = parent.type();
if ( /* parentType instanceof TupleType */) {
    if (property.equals("left")) {
        return new Some(new Tuple2Impl(state, new IndexValue(parent, new SymbolValue("0", Int))));
    }
...
Option < [CompileState, string] > {
    return: suffix(input.strip(), ";", (withoutEnd()), {
        return /* parseDefinition */(state, withoutEnd) { }, : .map((result()), {
            let, generated() { }, createIndent(depth) { }
...
Option < [CompileState, Definition] > {
    return: last(input.strip(), " ", (beforeName(), name()), {
        return /* split */() { }
    }(), /* toLast */ (beforeName, " ", Main.foldTypeSeparator), (beforeType(), type()), {
        return: suffix(beforeType.strip(), ">", (withoutTypeParamStart()), {
            return: first(withoutTypeParamStart, "<", (beforeTypeParams(), typeParamsString()), {
                let
            }(state1(), s()), new Tuple2Impl(state1, s.strip())),
            let, typeParams() { }, parseValuesOrEmpty(state, typeParamsString) { }
        }(state1(), s()), new Some(compileStateStringTupleBiFunction(state1, s))),
        return /* assembleDefinition */(typeParams) { }
    }(), new Some(beforeTypeParams), name, typeParams(), type)
};
;
or(() => {
...
{
    let children;
    () = arguments.iterate().map(Main.retainType).flatMap(Iterators.fromOption).collect(new ListCollector());
...
Option < [CompileState, Type] > {
    return: suffix(input.strip(), ">", (withoutEnd()), {
        return: first(withoutEnd, "<", (base(), argumentsString()), {
            ...
            return: parseValues(state, argumentsString, Main.argument).map((argumentsTuple()), [CompileState, (List)])
        }, {
...
return parseType(state, input).map((tuple()), [CompileState, Type]);
new Tuple2Impl(tuple[0](), tuple[1]());
;
...
Option < T > {
    return: splitter().flatMap((splitTuple()), [string, string]), splitMapper(splitTuple, [], ) { }
}(), splitTuple[1]();
;
...
{
    let replaced;
    () = input.replace("/*", "content-start").replace("*/", "content-end");
    return "/* " + replaced + " */";
}
