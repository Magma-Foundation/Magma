...
/* private static final */ class JVMList {
    ...
    iterateWithIndices() {
        return new HeadedIterator(new RangeHead(elements.size())).map((index) => new /* Tuple2Impl */ (index, /* this */ .elements.get(index)));
    }
    removeFirst() {
        /* if (this.elements.isEmpty())  */ {
            return new None();
    ...
    addAllLast(others) {
        let initial =  /* this */;
        return others.iterate().fold(initial, addLast);
    }
    get(index) {
...
/* private */ class CompileState /*  */ {
    constructor(structures, definitions, objectTypes, maybeStructName, typeParams, typeRegister) {
    }
    resolveValue(name) {
        return /* this */ .definitions.iterate().filter((definition) => definition.name.equals(name)).next().map(type);
    }
    addStructure(structure) {
        return new CompileState(structures.addLast(structure), definitions, objectTypes, maybeStructName, typeParams, typeRegister);
    }
    withDefinitions(definitions) {
        return new CompileState(structures, definitions.addAllLast(definitions), objectTypes, maybeStructName, typeParams, typeRegister);
    }
    resolveType(name) {
    ...
    addType(type) {
        return new CompileState(structures, definitions, objectTypes.addLast(type), maybeStructName, typeParams, typeRegister);
    }
    withDefinition(definition) {
        return new CompileState(structures, definitions.addLast(definition), objectTypes, maybeStructName, typeParams, typeRegister);
    }
    withStructName(name) {
        return new CompileState(structures, definitions, objectTypes, new Some(name), typeParams, typeRegister);
    }
    withTypeParams(typeParams) {
        return new CompileState(structures, definitions, objectTypes, maybeStructName, typeParams.addAllLast(typeParams), typeRegister);
    }
    withExpectedType(type) {
        return new CompileState(structures, definitions, objectTypes, maybeStructName, typeParams, new Some(type));
    }
}
...
/* private static */ class Iterators /*  */ {
    fromOption(option) {
        ...
        return new HeadedIterator(single.orElseGet(new ));
    }
}
    ...
    return: first(input, "(", (definitionString, withParams) => {
        return first(withParams, ")", (parametersString, rawContent) => {
            return /* parseDefinition */ (state, definitionString).flatMap((definitionTuple) => {
                ...
                let paramTypes = parameters.iterate().map() /* Definition */.type;
...
[CompileState, string];
{
    ...
    return first(stripped, "=", (definitionString, valueString) => {
        return /* parseDefinition */ (state, definitionString).flatMap((definitionTuple) => {
            let definitionState = definitionTuple.left();
            let definition = definitionTuple.right();
            let valueTuple = /* compileValue */ (definitionState, valueString, depth);
...
[CompileState, Type];
{
    ...
    /* if (base.equals("BiFunction"))  */ {
        return new Tuple2Impl(state, new FunctionType(of(children.get(0).orElse( /* null */), children.get(1).orElse( /* null */)), children.get(2).orElse( /* null */)));
    }
    /* if (base.equals("Function"))  */ {
        return new Tuple2Impl(state, new FunctionType(of(children.get(0).orElse( /* null */)), children.get(1).orElse( /* null */)));
    }
    /* if (base.equals("Predicate"))  */ {
        return new Tuple2Impl(state, new FunctionType(of(children.get(0).orElse( /* null */)), Boolean));
    }
