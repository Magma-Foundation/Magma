...
/* private */class CompileState/*  */ {
	...
	public resolveType(name : string) : Option<Type> {
		let maybe : Option<[string, List<string>]> = this.structNames.last().filter((inner) => inner.left() === name);
...
/* private */class StructurePrototype/*  */ implements IncompleteClassSegment {
	...
	generateToEnum() : string {
		...
		let joined : string = generateEnumEntries(variants);
...
/* public */class Main/*  */ {
	...
	static retainObjectRefType(type : Type) : Option<ObjectRefType> {
		...
		if (type._Variant === Variant.){
			let structureRefType : ObjectRefType = type as ObjectRefType;
	...
	parseStructureWithMaybeImplements(targetInfix : string, state : CompileState, beforeInfix : string, beforeContent : string, content1 : string, variants : List<string>, annotations : List<string>) : Option<[CompileState, IncompleteClassSegment]> {
		return this.first(beforeContent, " implements ", (s, s2) => {
			let stringList : List<TypeRef> = this.parseTypeRefs(s2);
	...
	completeStructure(state : CompileState, prototype : StructurePrototype) : Option<[CompileState, ClassSegment]> {
		...
		return this.resolveTypeRefs(withThis, prototype.interfaces).flatMap((interfacesTuple) => {
			return this.resolveTypeRefs(interfacesTuple.left(), prototype.superTypes).flatMap((superTypesTuple) => {
				...
				let variantsSuper : List<string> = this.findBaseNamesOfVariants(left, bases, prototype.name);
	...
	completeStructureWithStatements(prototype : StructurePrototype, variantsSuper : List<string>, thisType : StructureType, interfaces : List<Type>) : (arg0 : [CompileState, List<ClassSegment>]) => [CompileState, ClassSegment] {
		/* return oldStatementsTuple -> */{
			...
			let withEnumCategoriesDefinedTuple : [CompileState, List<ClassSegment>] = this.defineEnumCategories(exited, oldStatements, prototype.name, prototype.variants, prototype.generateToEnum());
			...
			let withEnumCategoriesImplemented : List<ClassSegment> = this.implementEnumCategories(prototype.name, variantsSuper, withEnumCategoriesDefined);
			let withConstructor : List<ClassSegment> = this.defineConstructor(withEnumCategoriesImplemented, prototype.parameters());
			let withEnumValues : List<ClassSegment> = this.implementEnumValues(withConstructor, thisType);
			let generatedSegments : string = this.joinSegments(withEnumValues);
			let joinedTypeParams = prototype.joinTypeParams();
			let interfacesJoined : string = this.joinInterfaces(interfaces);
			let generatedSuperType : string = this.joinSuperTypes(withEnumCategoriesDefinedState, prototype);
	...
	defineEnumCategories(state : CompileState, segments : List<ClassSegment>, name : string, variants : List<string>, enumGenerated : string) : [CompileState, List<ClassSegment>] {
		...
		let enumType : ObjectRefType = new ObjectRefType(name + "Variant");
		let enumDefinition : Definition = this.createVariantDefinition(enumType);
	...
	implementEnumCategories(name : string, variantsBases : List<string>, oldStatements : List<ClassSegment>) : List<ClassSegment> {
		return variantsBases.iterate().fold(oldStatements, (classSegmentList, superType) => {
			...
			let variantType : ObjectRefType = new ObjectRefType(variantTypeName);
			let definition : Definition = this.createVariantDefinition(variantType);
			let source : SymbolValue = new SymbolValue(variantTypeName + "." + name);
			let initialization : FieldInitialization = new FieldInitialization(definition, source);
	...
	flattenEnumValues(segment : ClassSegment, thisType : StructureType) : Query<ClassSegment> {
		if (segment._Variant === Variant.){
			...
			return enumValues.values.iterate().map((enumValue) => {
				let definition : Definition = new Definition(Lists.empty(), Lists.of("static"), enumValue.value, thisType, Lists.empty());
	...
	resolveInvokable(state : CompileState, invokable : Invokable) : Type {
		...
		/* return switch (caller) */{
			...
			/* case Value value1 -> */{
				let type : Type = this.resolveValue(state, value1);
				if (type._Variant === Variant.){
	...
	resolveDataAccess(state : CompileState, dataAccess : DataAccess) : Type {
		let parentType : Type = this.resolveValue(state, dataAccess.parent);
	...
	parseInstanceOf(state : CompileState, input : string, depth : number) : Option<[CompileState, Value]> {
		return this.last(input, "instanceof", (beforeKeyword, afterKeyword) => {
			let childTuple : [CompileState, Value] = this.parseValue(state, beforeKeyword, depth);
	...
	parseInstanceOfWithValue(depth : number, child : Value) : (arg0 : [CompileState, Definition]) => [CompileState, Value] {
		/* return definitionTuple -> */{
			...
			let type : Type = this.resolveValue(definitionSTate, child);
			let variant : DataAccess = new DataAccess(child, "_" + type.findName() + "Variant");
			let generate = type.findName();
			let temp : SymbolValue = new SymbolValue(generate + "Variant." + definition.findType().findName());
			let functionSegment : Statement = new Statement(depth + 1, new Initialization(definition, new Cast(child, definition.findType())));
	...
	assembleInvokable(state : CompileState, depth : number, argumentsString : string, callerString : string) : Some<[CompileState, Value]> {
		...
		let callerType : FunctionType = this.findCallerType(newCaller, oldCallerState);
		...
		let arguments : List<Value> = this.retainValues(argumentsWithActualTypes.iterate().map(Tuple2.left).collect(new ListCollector()));
		...
		if (newCaller._Variant === Variant.){
			if (value._Variant === Variant.){
				...
				let parentType : Type = this.resolveValue(argumentsState, parent);
	...
	findCallerType(newCaller : Caller, state : CompileState) : FunctionType {
		...
		/* switch (newCaller) */{
			...
			/* case Value value -> */{
				let type : Type = this.resolveValue(state, value);
				if (type._Variant === Variant.){
					let functionType : FunctionType = type as FunctionType;
	...
	modifyCaller(state : CompileState, oldCaller : Caller) : Caller {
		if (oldCaller._Variant === Variant.){
			let type : Type = this.resolveValue(state, access.parent);
	...
	parseDataAccess(state : CompileState, input : string, depth : number) : Option<[CompileState, Value]> {
		return this.last(input.strip(), ".", (parentString, rawProperty) => {
			...
			let parent = tuple[1];
			let parentType : Type = this.resolveValue(tuple[0], parent);
			let type : Type = Primitive.Unknown;
			if (parentType._Variant === Variant.){
	...
	parseClassInitialization(depth : number, state : CompileState, withoutEnd : string) : Option<[CompileState, IncompleteClassSegment]> {
		return this.first(withoutEnd, "=", (s, s2) => {
			return this.parseDefinition(state, s).map((result) => {
				let valueTuple : [CompileState, Value] = this.parseValue(result.left(), s2, depth);
	...
	assembleDefinition(state : CompileState, annotations : List<string>, modifiers : List<string>, rawName : string, typeParams : List<string>, type : string) : Option<[CompileState, Definition]> {
		return this.parseType(state.withTypeParams(typeParams), type).flatMap((type1) => {
			...
			let node : Definition = new Definition(annotations, newModifiers, stripped, type1.right(), typeParams);
