...
export class FoldingDivider {
	...
	public divide( input : String) : Iter<String> { let current : DivideState=new MutableDivideState( input);if(true){  let maybePopped : var=current.pop( );if(true){ break;;}current=maybePopped.orElse( null).left( ); let c : char=maybePopped.orElse( null).right( ); let finalCurrent : var=current;current=FoldingDivider.foldSingleQuotes( current, c).or( 0).orElseGet( 0);;}return current.advance( ).iter( );;}
