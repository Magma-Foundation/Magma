...
export class DividingSplitter {
	...
	Last(divider : Divider, delimiter : String) : Splitter {return new DividingSplitter( divider, new LastSelector( delimiter));;}
	First(divider : Divider, delimiter : String) : Splitter {return new DividingSplitter( divider, new FirstSelector( delimiter));;}
	split(input : String) : Option<Tuple2<String, String>> {list : List<String>=this.divider.divide( input).collect( new ListCollector<>( ));return this.selector.select( list);;}
