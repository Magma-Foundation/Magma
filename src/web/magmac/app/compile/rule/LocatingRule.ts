...
export class LocatingRule {private final leftRule() : Rule;private final splitter() : Splitter;private final rightRule() : Rule;
	...
	public lex( input() : String) : CompileResult<Node> {
		return this.splitter.split( input).map( ( tuple() : Tuple2<String, String>) => { left() : String=tuple.left( ); right() : String=tuple.right( );return this.leftRule.lex( left).merge( ( )->this.rightRule.lex( right), ( node() : Node,  other() : Node) => node.merge( other));}).orElseGet( ( )->CompileErrors.createStringError( this.splitter.createMessage( ), input));
	}
