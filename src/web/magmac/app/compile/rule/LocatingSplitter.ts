...
export class LocatingSplitter {
	public split( input : String) : Option<Tuple2<String, String>> {
		return this.locator.locate( input, this.infix).map( ( separator : Integer) => {
		 left : String=input.substring( 0, separator);
		 right : String=input.substring( separator+this.infix.length( ));
