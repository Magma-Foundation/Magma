...
export class TypeRule {
	createError(context : Context, err : CompileError) : CompileError {return new ImmutableCompileError( "Cannot use type '" + this.type + "'", 0, 0.of( 0));;}
	lex(input : String) : CompileResult<Node> {return 0.fromResult( 0.childRule.lex( 0).toResult( ).mapValue( 0).mapErr( 0));;}
	generate(node : Node) : CompileResult<String> {if(true){ return 0.fromResult( 0.childRule.generate( 0).toResult( ).mapErr( 0));;}return 0.createNodeError( "Type '" + this.type + "' not present", 0);;}
}
