...
export class TypeRule {
	...
	public lex( input : String) : CompileResult<Node> {
		return CompileResults.fromResult( this.childRule.lex( input).result( ).mapValue( ( node : Node) => node.retype( this.type)).mapErr( ( err : CompileError) => this.createError( new StringContext( input), err)));
	}
	public generate( node : Node) : CompileResult<String> {
		if(node.is( this.type)){ 
		return CompileResults.fromResult( this.childRule.generate( node).result( ).mapErr( ( err : CompileError) => this.createError( new NodeContext( node), err)));}
