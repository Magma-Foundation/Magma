...
export class MapNode {private final maybeType : Option<String>;private nodeLists : Map<String, NodeList>;private nodes : Map<String, Node>;private strings : Map<String, String>;
	...
	private static fold( node : Node,  iter : Iter<Tuple2<String, T>>,  mapper : Function<Node, BiFunction<String, T, Node>>) : Node {
		return iter.fold( node, ( current : Node,  tuple : Tuple2<String, T>) => {
		 left : String=tuple.left( );
		 right : T=tuple.right( );
		return mapper.apply( current).apply( left, right);});
	}
	private static formatNodeList( depth : int,  nodeList : NodeList) : String {
	...
	private static formatEntry( depth : int,  key : String,  value : String) : String {
		 indent : String=MapNode.createIndent( depth+1);
		return indent+key+": "+value;
	}
	public iterNodes() : Iter<Tuple2<String, Node>> {
	...
	public format( depth : int) : String {
		 ...
		 stringsStream : Iter<String>=MapNode.toStream( depth, this.strings, ( value : String) => "\"" + value + "\"");
		 nodesStream : Iter<String>=MapNode.toStream( depth, this.nodes, ( value : Node) => value.format( depth+1));
		 nodeListsStream : Iter<String>=MapNode.toStream( depth, this.nodeLists, ( values : NodeList) => "[" + MapNode.formatNodeList(depth, values) + "]");
	...
	private static toStream( depth : int,  map : Map<String, T>,  mapper : Function<T, String>) : Iter<String> {
		if(map.isEmpty( )){ 
		return Iters.empty( );}
		return map.iter( ).map( ( entry : Tuple2<String, T>) => {
		 key : String=entry.left( );
		 value : T=entry.right( );
		return MapNode.formatEntry( depth, key, mapper.apply( value));});
	}
	public withString( key : String,  value : String) : Node {
		this.strings=this.strings.put( key, value);
		return this;
	}
	public findString( key : String) : Option<String> {
	...
	public iterStrings() : Iter<Tuple2<String, String>> {
		return this.strings( ).iter( );
	}
	public hasNodeList( key : String) : boolean {
		return this.nodeLists.containsKey( key);
	}
	public iterNodeLists() : Iter<Tuple2<String, NodeList>> {
		return this.nodeLists.iter( );
	}
	public withNodeList( key : String,  values : NodeList) : Node {
	...
	public findNodeList( key : String) : Option<NodeList> {
