...
export class MapNode {
	...
	formatEntry(depth : int, key : String, value : String) : String {indent : var=MapNode.createIndent( depth+1);return indent+key+": "+value;;}
	...
	format(depth : int) : String {typeString : var=this.maybeType.map( 0).orElse( "");stringsStream : var=MapNode.toStream( depth, this.strings, 0);nodesStream : var=MapNode.toStream( depth, this.nodes, 0);nodeListsStream : var=MapNode.toStream( depth, this.nodeLists, 0);joined : var=stringsStream.concat( nodesStream).concat( nodeListsStream).collect( new Joiner( ", ")).orElse( "");return typeString+"{" + joined + MapNode.createIndent(depth) + "}";;}
	...
	hasContent() : boolean {return !this.strings.isEmpty( )||!this.nodes.isEmpty( )||!this.nodeLists.isEmpty( );;}
	withNodeListAndSerializer(key : String, list : List<T>, serializer : Function<T, Node>) : Node {nodeList : var=list.iter( ).map( serializer).collect( new NodeListCollector( ));return this.withNodeList( key, nodeList);;}
