...
export class JavaTypescriptParser {
	private static parseUnit( unit : Unit<JavaLang.JavaRoot>) : CompileResult<Unit<TypescriptLang.TypescriptRoot>> {return unit.destruct( JavaTypescriptParser.parseRoot);;}
	private static parseRoot( location : Location,  root : JavaLang.JavaRoot) : CompileResult<Unit<TypescriptLang.TypescriptRoot>> { let rootSegments : var=root.children( ).iter( ).map( 0).flatMap( List.iter).collect( new ListCollector<>( ));return CompileResults.Ok( new SimpleUnit<>( location, new TypescriptLang.TypescriptRoot( rootSegments)));;}
	...
	public apply( initial : UnitSet<JavaLang.JavaRoot>) : CompileResult<UnitSet<TypescriptLang.TypescriptRoot>> {return initial.iter( ).map( JavaTypescriptParser.parseUnit).collect( new CompileResultCollector<>( new UnitSetCollector<>( )));;}
}
