...
export class JavaTypescriptParser {
	...
	parseMethod(methodNode : JavaMethod) : TypescriptLang.TypescriptStructureMember {parameters : List<TypescriptLang.TypeScriptParameter>=methodNode.parameters( ).iter( ).map( JavaTypescriptParser.parseParameter).collect( new ListCollector<>( ));header : TypescriptLang.TypeScriptMethodHeader=JavaTypescriptParser.parseMethodHeader( methodNode.header( ));parameterizedHeader : ParameterizedMethodHeader<TypescriptLang.TypeScriptParameter>=new ParameterizedMethodHeader<>( header, parameters);return new TypescriptLang.TypescriptMethod( parameterizedHeader, methodNode.maybeChildren( ).map( JavaTypescriptParser.parseFunctionSegments));;}
	...
	parseDefinition(definition : JavaLang.Definition) : TypescriptLang.Definition {maybeAnnotations : Option<List<Annotation>>=definition.maybeAnnotations( );maybeModifiers : List<Modifier>=definition.modifiers( );maybeTypeParameters : Option<List<TypescriptLang.TypeParam>>=definition.maybeTypeParams( );type : JavaLang.JavaType=definition.type( );oldName : String=definition.name( );if(true){ newType : TypescriptLang.ArrayType=new TypescriptLang.ArrayType( JavaTypescriptParser.parseType( child));name : String="..."+oldName;return new TypescriptLang.Definition( maybeAnnotations, maybeModifiers, name, maybeTypeParameters, newType);;}if(true){ return new TypescriptLang.Definition( maybeAnnotations, maybeModifiers, oldName, maybeTypeParameters, JavaTypescriptParser.parseType( definition.type( )));;};}
	parseSymbol(symbol : JavaLang.Symbol) : Symbol {return new Symbol( symbol.value( ));;}
	parseQualifiedType(qualified : JavaLang.Qualified) : Symbol {joined : String=qualified.segments( ).iter( ).map( Segment.value).collect( new Joiner( ".")).orElse( "");return new Symbol( joined);;}
	...
	parseTemplateType(type : JavaLang.JavaTemplateType) : TypescriptLang.TemplateType {base : JavaLang.Symbol=JavaTypescriptParser.parseBaseType( type.base( ));collect : List<TypescriptLang.Type>=JavaTypescriptParser.parseTypeList( type.typeArguments( ).arguments( ));return new TypescriptLang.TemplateType( base, new TypeArguments<>( collect));;}
	...
	parseImport(location : Location, segments : List<Segment>) : TypescriptLang.TypeScriptImport {segmentValues : List<String>=segments.iter( ).map( Segment.value).collect( new ListCollector<>( ));before : List<Segment>=location.namespace( ).iter( ).map( 0).map( Segment.new).collect( new ListCollector<>( ));last : Segment=new Segment( segmentValues.findLast( ).orElse( ""));return new TypescriptLang.TypeScriptImport( Lists.of( last), before.addAllLast( segments));;}
