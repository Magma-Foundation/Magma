...
export class ImmutableCompileError {
	...
	public format( depth : int) : String { let copy : var=this.errors.sort( 0); let joined : var=ImmutableCompileError.joinSorted( depth, copy);return this.message+": "+this.context.display( )+joined;;}
	private static joinSorted( depth : int,  copy : List<CompileError>) : String {return copy.iter( ).map( 0).map( 0).collect( new Joiner( )).orElse( "");;}
