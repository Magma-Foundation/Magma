...
export class ImmutableCompileError {
	...
	formatEntry(depth : int, display : String) : String {repeated : var="| ".repeat( depth+1);return "\n"+repeated+display;;}
	display() : String {return this.format( 0);;}
	format(depth : int) : String {copy : var=this.errors.sort( 0);joined : var=ImmutableCompileError.joinSorted( depth, copy);return this.message+": "+this.context.display( )+joined;;}
	joinSorted(depth : int, copy : List<CompileError>) : String {return copy.iter( ).map( 0).map( 0).collect( new Joiner( )).orElse( "");;}
	computeMaxDepth() : int {compileErrorIter : var=this.errors.iter( );return 1+compileErrorIter.map( CompileError.computeMaxDepth).collect( new Max( )).orElse( 0);;}
}
