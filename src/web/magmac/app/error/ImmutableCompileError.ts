...
export class ImmutableCompileError {
	...
	formatEntry(depth : int, display : String) : String {break;return "\n"+repeated+display;;}
	display() : String {return this.format( 0);;}
	format(depth : int) : String {break;break;return this.message+": "+this.context.display( )+joined;;}
	joinSorted(depth : int, copy : List<CompileError>) : String {return copy.iter( ).map( 0).map( 0).collect( new Joiner( )).orElse( "");;}
	computeMaxDepth() : int {break;return 1+compileErrorIter.map( CompileError.computeMaxDepth).collect( new Max( )).orElse( 0);;}
}
