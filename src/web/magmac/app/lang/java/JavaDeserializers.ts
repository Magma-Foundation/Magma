...
export class JavaDeserializers {
	...
	deserializeInstanceOfDefinition(node : Node) : CompileResult<InstanceOfDefinition> {return Deserializers.orError( "instance-of-definition", node, Lists.of( Deserializers.wrap( JavaDeserializers.deserializeInstanceOfWithParameters), Deserializers.wrap( JavaDeserializers.deserializeInstanceOfWithName)));;}
	deserializeInstanceOfWithName(node : Node) : Option<CompileResult<InstanceOfDefinitionWithName>> {return Destructors.destructWithType( "with-name", node).map( 0);;}
	deserializeInstanceOfWithParameters(node : Node) : Option<CompileResult<InstanceOfDefinitionWithParameters>> {return Destructors.destructWithType( "with-parameters", node).map( 0);;}
