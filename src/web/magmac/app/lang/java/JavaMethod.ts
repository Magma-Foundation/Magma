...
export class JavaMethod {
	...
	createMethodRule(childRule : Rule) : Rule {header : var=new NodeRule( "header", new OrRule( Lists.of( JavaRules.createDefinitionRule( ), new TypeRule( "constructor", new StripRule( FilterRule.Symbol( new StringRule( "name")))))));parameters : var=JavaRules.createParametersRule( JavaRules.createDefinitionRule( ));content : var=CommonLang.Statements( "children", childRule);rightRule : Rule=new StripRule( new PrefixRule( "{", new SuffixRule( new StripRule( "", content, "after-children"), "}")));withParams : Rule=new OptionNodeListRule( "parameters", new SuffixRule( parameters, ");"), LocatingRule.First( parameters, ")", rightRule));return new TypeRule( "method", LocatingRule.First( header, "(", withParams));;}
}
