...
export class PlantUMLAfterPasser {
	...
	replaceRootChild(child : Node) : CompileResult<NodeList> {maybeExtended : CompileResult<NodeList>=PlantUMLAfterPasser.createInherits( child, "extended");maybeImplemented : CompileResult<NodeList>=PlantUMLAfterPasser.createInherits( child, "implemented");return maybeExtended.flatMapValue( 0);;}
	...
	pass(state : ParseState, node : Node) : ParseResult {if(true){ result : CompileResult<ParseUnit<Node>>=PlantUMLAfterPasser.replaceRootChildren( node).flatMapValue( 0);return new InlinePassResult( new Some<>( result));;}if(true){ child : String=node.findNodeList( "segments").orElse( InlineNodeList.empty( )).findLast( ).orElse( null).findString( "value").orElse( "");dependency : Node=new MapNode( "dependency").withString( "parent", state.findLocation( ).name( )).withString( "child", child);tuple : ParseUnit<Node>=new ParseUnitImpl<Node>( state, dependency);return new InlinePassResult( new Some<>( CompileResults.Ok( tuple)));;}return InlinePassResult.empty( );;}
