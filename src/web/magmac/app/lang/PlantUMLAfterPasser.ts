...
export class PlantUMLAfterPasser {
	...
	replaceRootChild(child : Node) : CompileResult<NodeList> {break;break;return maybeExtended.flatMapValue( 0);;}
	...
	pass(state : ParseState, node : Node) : ParseResult {if(true){ break;return new InlinePassResult( new Some<>( result));;}if(true){ break;break;break;return new InlinePassResult( new Some<>( CompileResults.Ok( tuple)));;}return InlinePassResult.empty( );;}
