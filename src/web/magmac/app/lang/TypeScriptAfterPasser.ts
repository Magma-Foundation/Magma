...
export class TypeScriptAfterPasser {
	private static passImport( state : ParseState,  node : Node) : Option<PassResult> {
		if(!node.is( "import")){ 
		return new None<>( );}
	...
	private static passMethod( state : ParseState,  node : Node) : Option<PassResult> {
		if(node.is( "method")){ 
	...
	private format( state : ParseState,  node : Node) : Option<PassResult> {
