...
export class MergeDiagram {
	...
	findChildrenWithDependencies(rootSegments : NodeList) : Map<String, List<String>> {
		return rootSegments.iter( ).fold( Maps.empty( ), (current : Map<String, List<String>>, node : Node) => {if(node.is( "dependency")){  String parent=node.findString( "parent").orElse( ""); String child=node.findString( "child").orElse( "");return current.mapOrPut( parent, (stringList : List<String>) => stringList.add( child), ( )->Lists.of( child));}return current;});
	}
	findChildrenWithInheritedTypes(rootSegments : NodeList) : Map<String, List<String>> {
		return rootSegments.iter( ).fold( Maps.empty( ), (current : Map<String, List<String>>, node1 : Node) => {if(!node1.is( "inherits")){ return current;} String parent=node1.findString( "parent").orElse( ""); String child=node1.findString( "child").orElse( "");return current.mapOrPut( child, (stringList : List<String>) => stringList.add( parent), ( )->Lists.of( parent));});
	}
	afterAll(roots : Map<Location, Node>) : Map<Location, Node> {
		 NodeList oldRootSegments=new InlineNodeList( roots.iterEntries( ).map( (locationNodeTuple2 : Tuple2<Location, Node>) => locationNodeTuple2.right( )).map( (node : Node) => node.findNodeList( "children")).flatMap( (option : Option<NodeList>) => Iters.fromOption( option)).flatMap( (nodeList : NodeList) => nodeList.iter( )).collect( new ListCollector<>( )));
		 ...
		 NodeList newDependencies=childrenWithDependencies.iterEntries( ).fold( InlineNodeList.empty( ), (current : NodeList, entry : Tuple2<String, List<String>>) => { String child=entry.left( ); List<String> currentDependencies=entry.right( ); List<String> parentDependencies=MergeDiagram.findParentDependencies( child, childrenWithInheritedTypes, childrenWithDependencies); List<String> childWithInheritedTypes=childrenWithInheritedTypes.getOrDefault( child, Lists.empty( )); List<String> toRemove=parentDependencies.addAll( childWithInheritedTypes); List<String> list=currentDependencies.removeAll( toRemove); NodeList others=new InlineNodeList( list.iter( ).map( (parent : String) => new MapNode( "dependency").withString( "parent", parent).withString( "child", child)).collect( new ListCollector<>( )));return current.addAll( others);});
		 NodeList withoutDependencies=new InlineNodeList( oldRootSegments.iter( ).filter( (child : Node) => !child.is( "dependency")).collect( new ListCollector<>( )));
