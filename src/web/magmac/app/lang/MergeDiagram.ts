...
export class MergeDiagram {
	...
	private static findChildrenWithDependencies( rootSegments : NodeList) : Map<String, List<String>> {
		return rootSegments.iter( ).fold( Maps.empty( ), ( current : Map<String, List<String>>,  node : Node) => {
		if(node.is( "dependency")){ 
		 parent : String=node.findString( "parent").orElse( "");
		 child : String=node.findString( "child").orElse( "");
		return current.mapOrPut( parent, ( stringList : List<String>) => stringList.add( child), ( )->Lists.of( child));}
		return current;});
	}
	private static findChildrenWithInheritedTypes( rootSegments : NodeList) : Map<String, List<String>> {
		return rootSegments.iter( ).fold( Maps.empty( ), ( current : Map<String, List<String>>,  node1 : Node) => {
		if(!node1.is( "inherits")){ 
		return current;}
		 parent : String=node1.findString( "parent").orElse( "");
		 child : String=node1.findString( "child").orElse( "");
		return current.mapOrPut( child, ( stringList : List<String>) => stringList.add( parent), ( )->Lists.of( parent));});
	}
	private static createInitial() : MapUnitSet<Node> {
		return new MapUnitSet<>( );
	}
	public afterAll( roots : UnitSet<Node>) : UnitSet<Node> {
		 oldRootSegments : NodeList=new InlineNodeList( roots.iterValues( ).map( ( node : Node) => node.findNodeList( "children")).flatMap( ( option : Option<NodeList>) => Iters.fromOption( option)).flatMap( ( nodeList : NodeList) => nodeList.iter( )).collect( new ListCollector<>( )));
		 ...
		 newDependencies : NodeList=childrenWithDependencies.iter( ).fold( InlineNodeList.empty( ), ( current : NodeList,  entry : Tuple2<String, List<String>>) => {
		 child : String=entry.left( );
		 currentDependencies : List<String>=entry.right( );
		 parentDependencies : List<String>=MergeDiagram.findParentDependencies( child, childrenWithInheritedTypes, childrenWithDependencies);
		 childWithInheritedTypes : List<String>=childrenWithInheritedTypes.getOrDefault( child, Lists.empty( ));
		 toRemove : List<String>=parentDependencies.addAll( childWithInheritedTypes);
		 list : List<String>=currentDependencies.removeAll( toRemove);
		 others : NodeList=new InlineNodeList( list.iter( ).map( ( parent : String) => new MapNode( "dependency").withString( "parent", parent).withString( "child", child)).collect( new ListCollector<>( )));
