...
export class MergeDiagram {
	...
	afterAll(roots : UnitSet<Node>) : UnitSet<Node> {oldRootSegments : NodeList=new InlineNodeList( roots.iterValues( ).map( 0).flatMap( Iters.fromOption).flatMap( NodeList.iter).collect( new ListCollector<>( )));childrenWithInheritedTypes : Map<String, List<String>>=MergeDiagram.findChildrenWithInheritedTypes( oldRootSegments);childrenWithDependencies : Map<String, List<String>>=MergeDiagram.findChildrenWithDependencies( oldRootSegments);newDependencies : NodeList=childrenWithDependencies.iter( ).fold( InlineNodeList.empty( ), 0);withoutDependencies : NodeList=new InlineNodeList( oldRootSegments.iter( ).filter( 0).collect( new ListCollector<>( )));copy : NodeList=InlineNodeList.empty( ).add( new MapNode( "start")).addAll( withoutDependencies).addAll( newDependencies).add( new MapNode( "end"));node : Node=new MapNode( );root : Node=node.withNodeList( "children", copy);location : Location=new Location( Lists.empty( ), "diagram");return MergeDiagram.createInitial( ).add( new SimpleUnit<>( location, root));;}
}
