...
export class JavaLang {
	...
	createStructureStatementRule(value1 : LazyRule) : Rule {
		 Rule definition=new OrRule( Lists.of( new NodeRule( "value", new TypeRule( "definition", CommonLang.createDefinitionRule( ))), CommonLang.createAssignmentRule( value1)));
		return new TypeRule( "statement", new StripRule( new SuffixRule( definition, ";")));
	}
	createMethodRule(childRule : Rule) : Rule {
		 NodeRule header=new NodeRule( "header", new OrRule( Lists.of( CommonLang.createDefinitionRule( ), new TypeRule( "constructor", new StripRule( FilterRule.Symbol( new StringRule( "name")))))));
