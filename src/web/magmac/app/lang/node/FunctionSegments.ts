...
export class FunctionSegments {
	...
	public static initFunctionSegmentRule( functionSegmentRule : LazyRule,  value : Rule,  definition : Rule) : Rule { let functionSegmentValueRule : var=FunctionSegmentValues.createFunctionSegmentValueRule( value, definition); let rule : Rule=new OrRule( Lists.of( new TypeRule( "whitespace", new StripRule( new ExactRule( ";"))), JavaRules.createWhitespaceRule( ), JavaRules.createStatementRule( functionSegmentValueRule), JavaRules.createBlockRule( functionSegmentRule, value, definition), JavaRules.createReturnRule( value), JavaRules.createCaseRule( value, functionSegmentRule)));return functionSegmentRule.set( new StripRule( "before", rule, ""));;}
}
