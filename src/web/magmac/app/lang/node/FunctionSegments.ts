...
export class FunctionSegments {
	...
	initFunctionSegmentRule(functionSegmentRule : LazyRule, value : Rule, definition : Rule) : Rule {functionSegmentValueRule : Rule=FunctionSegmentValues.createFunctionSegmentValueRule( value, definition);rule : Rule=new OrRule( Lists.of( new TypeRule( "whitespace", new StripRule( new ExactRule( ";"))), JavaRules.createWhitespaceRule( ), JavaRules.createStatementRule( functionSegmentValueRule), JavaRules.createBlockRule( functionSegmentRule, value, definition), JavaRules.createReturnRule( value), JavaRules.createCaseRule( value, functionSegmentRule)));return functionSegmentRule.set( new StripRule( "before", rule, ""));;}
}
