...
export class TreeParser {
	...
	parseNodeListEntry(current : ParseUnit<Node>, entry : Tuple2<String, NodeList>) : CompileResult<ParseUnit<Node>> {currentNode : var=current.right( );key : var=entry.left( );initial : var=CompileResults.Ok( current.retainWithList( ));return entry.right( ).iter( ).fold( initial, 0).mapValue( 0);;}
	...
	parseNodeEntry(current : ParseUnit<Node>, entry : Tuple2<String, Node>) : CompileResult<ParseUnit<Node>> {currentNode : var=current.right( );key : var=entry.left( );parsed : var=this.parseTree( current.left( ), entry.right( ));return parsed.mapValue( 0);;}
