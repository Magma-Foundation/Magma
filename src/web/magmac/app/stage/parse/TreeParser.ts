...
export class TreeParser {
	...
	parseNodeListEntry(current : ParseUnit<Node>, entry : Tuple2<String, NodeList>) : CompileResult<ParseUnit<Node>> {currentNode : Node=current.right( );key : String=entry.left( );initial : CompileResult<ParseUnit<NodeList>>=CompileResults.Ok( current.retainWithList( ));return entry.right( ).iter( ).fold( initial, 0).mapValue( 0);;}
	...
	parseNodeEntry(current : ParseUnit<Node>, entry : Tuple2<String, Node>) : CompileResult<ParseUnit<Node>> {currentNode : Node=current.right( );key : String=entry.left( );parsed : CompileResult<ParseUnit<Node>>=this.parseTree( current.left( ), entry.right( ));return parsed.mapValue( 0);;}
