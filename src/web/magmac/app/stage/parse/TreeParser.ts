...
export class TreeParser {
	...
	private parseTree( state : ParseState,  root : Node) : CompileResult<ParseUnit<Node>> {return this.beforeChild.pass( state, root).orElseGet( ( )->new ParseUnitImpl<Node>( state, root)).flatMapValue( this.parseNodeLists).flatMapValue( this.parseNodes).flatMapValue( 0);;}
