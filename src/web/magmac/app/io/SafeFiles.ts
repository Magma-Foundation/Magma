...
export class SafeFiles {
	...
	walk(sourceDirectory : Path) : IOResult<Iter<Path>> {if(true){ stream : var=Files.walk( sourceDirectory);return new InlineIOResult<>( new Ok<>( new JVMList<>( stream.collect( Collectors.toList( ))).iter( )));;}if(true){ return new InlineIOResult<>( new Err<>( e));;};}
