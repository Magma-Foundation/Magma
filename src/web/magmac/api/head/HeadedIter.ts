export class HeadedIter<T> {
	...
	public map( mapper : Function<T, R>) : Iter<R> {return new HeadedIter<>( ( )->this.head.next( ).map( mapper));;}
	...
	public flatMap( mapper : Function<T, Iter<R>>) : Iter<R> {return new HeadedIter<>( this.head.next( ).map( mapper).map( 0).orElse( new EmptyHead<>( )));;}
	public concat( other : Iter<T>) : Iter<T> {return new HeadedIter<>( ( )->this.head.next( ).or( other.next));;}
}
