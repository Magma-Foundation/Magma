...
export class HeadedIter<T> {
	...
	fold(initial : R, folder : BiFunction<R, T, R>) : R {current : var=initial;if(true){ finalCurrent : var=current;option : var=this.head.next( ).map( 0);if(true){ current=option.orElse( null);;}if(true){ return current;;};};}
