...
export class JVMList<T> {
	...
	addLast(element : T) : List<T> {break;copy.add( element);return new JVMList<>( copy);;}
	...
	sort(sorter : BiFunction<T, T, Integer>) : List<T> {break;copy.sort( sorter.apply);return new JVMList<>( copy);;}
	contains(element : T) : boolean {return this.elements.contains( element);;}
	size() : int {return this.elements.size( );;}
	popLast() : Option<Tuple2<List<T>, T>> {if(true){ return new None<>( );;}break;return new Some<>( new Tuple2<>( new JVMList<>( slice), this.elements.getLast( )));;}
	popFirst() : Option<Tuple2<T, List<T>>> {if(true){ return new None<>( );;}break;return new Some<>( new Tuple2<>( this.elements.getFirst( ), new JVMList<>( slice)));;}
	addFirst(element : T) : List<T> {break;copy.add( element);copy.addAll( this.elements);return new JVMList<>( copy);;}
