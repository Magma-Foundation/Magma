...
export class Main {
	static main(): void {
		...
		sourceDirectory.walk().match((children: List<Path>) => Main.runWithChildren(children, sourceDirectory)/*unknown*/, (value: IOError) => new Some<IOError>(value)/*unknown*/).map((error: IOError) => error.display()/*unknown*/).ifPresent((displayed: string) => Console.printErrLn(displayed)/*unknown*/)/*unknown*/;
	}
	...
	static retainDefinitionsFromParameters(parameters: List<Parameter>): List<Definition> {
		return parameters.query().map((parameter: Parameter) => parameter.asDefinition()/*unknown*/).flatMap(Iterators.fromOption).collect(new ListCollector<Definition>())/*unknown*/;
	}
	...
	static or<T>(state: CompileState, input: string, rules: List<(arg0 : CompileState, arg1 : string) => Option<Tuple2<CompileState, T>>>): Option<Tuple2<CompileState, T>> {
		return rules.query().map((rule: (arg0 : CompileState, arg1 : string) => Option<Tuple2<CompileState, T>>) => Main.getApply(state, input, rule)/*unknown*/).flatMap(Iterators.fromOption).next()/*unknown*/;
	}
	...
	static retain<T, R>(args: List<T>, mapper: (arg0 : T) => Option<R>): List<R> {
		return args.query().map(mapper).flatMap(Iterators.fromOption).collect(new ListCollector<R>())/*unknown*/;
	}
	...
	static isNumber(input: string): boolean {
		let query = new HeadedQuery<number>(new RangeHead(Strings.length(input)))/*unknown*/;
